...
public class AbstractStormObject extends AbstractWeatherObject implements IWeatherRain, IWeatherLayered
{
	...
	public void tickWeatherEvents()
	{
		...
		if (stage > Stage.RAIN.getStage() && Maths.random(0, ConfigStorm.lightning_bolt_1_in_x - (int)(ConfigStorm.lightning_bolt_1_in_x * lightning)) == 0)
		{
			...
			if (world.isLoaded(blockPos))
				createLightning(pos.posX, (double) world.getHeight(Heightmap.Type.MOTION_BLOCKING, (int) Math.round(pos.posX), (int) Math.round(pos.posZ)), pos.posZ, true);
			else
		...
		for (int i = 0; i < world.players().size(); i++)
		{
			...
			if (pos.distance(player.getX(), pos.posY, player.getZ()) < size)
			{
				if (isHailing())
				{
	...
	public void tickSnowFall()
	{
		...
		for (xx = (int) (pos.posX - size/2); xx < pos.posX + size/2; xx+=16)
		{
			for (zz = (int) (pos.posZ - size/2); zz < pos.posZ + size/2; zz+=16)
			{
				...
				if (WeatherUtil.canDoRainSnowIce(world, chunk) && (ConfigSnow.Snow_RarityOfBuildup == 0 || world.random.nextInt(ConfigSnow.Snow_RarityOfBuildup) == 0))
				{
					...
					if ((double)MathHelper.sqrt(d0 * d0 + d2 * d2) > size)
						continue;
					setBlockHeight = world.getHeight(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, xxx + x, zzz + z);
					if (canSnowAtBody(xxx + x, setBlockHeight, zzz + z) && Blocks.SNOW.defaultBlockState().canSurvive(world, new BlockPos(xxx + x, setBlockHeight, zzz + z)))
					{
						boolean betterBuildup = true;
						
						if (betterBuildup)
						{
							AbstractWindManager windMan = manager.windManager;
	...
	public CoroChunkCoordsBlock getSnowfallEvenOutAdjustCheck(int x, int y, int z, int sourceMeta)
	{
		//filter out diagonals
		CoroChunkCoordsBlock attempt;
		...
		return new CoroChunkCoordsBlock(0, 0, 0, Blocks.AIR, 0);
	}
	...
	public CoroChunkCoordsBlock getSnowfallEvenOutAdjust(int x, int y, int z, int sourceMeta)
	{
		...
		if (WeatherUtilBlock.isAir(checkID))
		{
			...
			if (WeatherUtilBlock.isAir(checkID2))
				return new CoroChunkCoordsBlock(0, 0, 0, Blocks.AIR, 0); 
			else
				//return that its an open area to start snow at
				return new CoroChunkCoordsBlock(x, y, z, Blocks.AIR, 0);
		}
		else if (checkID == Blocks.SNOW)
		{
			BlockState state = world.getBlockState(new BlockPos(x, y, z));
			int checkMeta = state.getValue(BlockStateProperties.LAYERS);
			
			//if detected snow is shorter, return with detected meta val!
			//adjusting to <=
			if (checkMeta < sourceMeta)
				return new CoroChunkCoordsBlock(x, y, z, checkID, checkMeta);
		}
		else
			return new CoroChunkCoordsBlock(0, 0, 0, Blocks.AIR, 0);
	...
	public boolean canSnowAtBody(int par1, int par2, int par3)
	{
		...
		else
		{
			if (par2 >= 0 && par2 < 256 && world.getBrightness(LightType.BLOCK, pos) < 10)
			{
				...
				if ((iblockstate1.getBlock().isAir(iblockstate1, world, pos) || iblockstate1.getBlock() == Blocks.SNOW) && Blocks.SNOW.defaultBlockState().canSurvive(world, pos))
					return true;
			}
	...
	public void tickProgressionNormal()
	{
		...
		if (ticks % ConfigStorm.storm_tick_delay == 0)
		{
			...
			hasWater = blockID.getMaterial() == Material.WATER;
			...
			if (isStorm())
			{
				if (shouldBuildHumidity)
				{
					...
					if (rain < 0.0F)
					{
						Weather2Remastered.debug("Storm " + getUUID().toString() + " has stopped raining");
						rain = 0.0F;
				...
				if (!isDying)
					...
					{
						Weather2Remastered.debug("Storm " + getUUID().toString() + " was forced to dissipate because of overcast mode at stage " + stage + " and is now dying");
						isDying = true;
					}
					...
					{
						Weather2Remastered.debug("Storm " + getUUID().toString() + " was forced to dissipate because it was disabled at stage " + stage + " and is now dying");
						isDying = true;
					}
				...
				if (!isDying)
				{
					...
					if (intensify && (stage < stageMax || alwaysProgresses))
					{
						...
						{
							Weather2Remastered.debug("Storm " + getUUID().toString() + " was converted into a tropical cyclone");
					...
					{
						Weather2Remastered.debug("Storm " + getUUID().toString() + " has peaked at stage " + stage + " and is now dying");
						isDying = true;
					}
				}
				else
				{
					...
					{
						stagePrev();
						Weather2Remastered.debug("Storm " + getUUID().toString() + " has weakened to stage " + stage);
	...
	public void resetStorm()
	{
		...
		Biome biome = world.getBiome(new BlockPos(MathHelper.floor(pos.posX), 0, MathHelper.floor(pos.posZ)));
		if (shouldConvert) {
			stormType = biome != null && biome.getBiomeCategory().toString().contains("ocean") ? StormType.WATER.ordinal() : StormType.LAND.ordinal();
			Weather2Remastered.debug("Storm in " + biome.getBiomeCategory().toString() + "returned stormType: " + stormType);
		}
		if (isViolent)
		{
	...
	public void setNoStorm() {
		Weather2Remastered.debug("Storm " + this.getUUID().toString() + " was terminated");
	...
	public void spinEntity(Object obj)
	{
		...
		float heightMult = getLayerHeight() * (world.isClientSide() && obj instanceof Particle ? 0.004F : 0.0034F);
		float rotationMult = heightMult * 0.5F * ((isViolent ? 3.1F : 1.55F) + Math.min((stage - 5.0F) / 3.0F, 2.0F));
		World world = CoroEntParticle.getWorld(obj);
		...
		double radius = 10D, scale = conf.tornadoWidthScale * (world.isClientSide() && obj instanceof Particle ? 1D : 8.0D);
		double d1 = pos.posX - CoroEntParticle.getPosX(obj), d2 = pos.posZ - CoroEntParticle.getPosZ(obj);
		...
		if (conf.type == WeatherEntityConfig.TYPE_SPOUT)
		{
			...
			float heightPercent = (float) (1F - ((CoroEntParticle.getPosY(obj) - posGround.posY) / (pos.posY - posGround.posY)));
		...
		double distY = pos.posY - CoroEntParticle.getPosY(obj);
		double distXZ = Math.sqrt(Math.abs(d1)) + Math.sqrt(Math.abs(d2));

		if (CoroEntParticle.getPosY(obj) - pos.posY < 0.0D)
			distY = 1.0D;
		else
			distY = CoroEntParticle.getPosY(obj) - pos.posY;
		...
		if (obj instanceof PlayerEntity)
		{
			...
			if (CoroEntParticle.getMotionY(obj) > 0.0D)
				ent.fallDistance = 0F;
		}
		else if (obj instanceof LivingEntity)
		{
			airTime = ent.getPersistentData().getInt("timeInAir") + 1.0D;

			if (ent.getDeltaMovement().y > 0.0D) ent.fallDistance = 0F;
		...
		if (conf.type != 0 && (obj instanceof LivingEntity))
			f5 *= ent.isOnGround() ? 2F : 7.0F;
	...
	{
		CoroEntParticle.setMotionX(entity, CoroEntParticle.getMotionX(entity) + f);
		CoroEntParticle.setMotionY(entity, CoroEntParticle.getMotionY(entity) + f1);
		CoroEntParticle.setMotionZ(entity, CoroEntParticle.getMotionZ(entity) + f2);

		if (entity instanceof SquidEntity)
		{
			Entity ent = (Entity) entity;
			ent.setPos(ent.getX() + ent.getDeltaMovement().x * 5F, ent.getY(), ent.getZ() + ent.getDeltaMovement().z * 5F);
		}
	}
	...
	public void aimStormAtPlayer(PlayerEntity entP)
	{
		...
		if (entP != null)
		{
			...
			Weather2Remastered.debug("Storm " + getUUID() + " was aimed at player " + WeatherUtilEntity.getName(entP));
		}
	}
	...
	{
		float distance = Math.max((float) Math.sqrt(pos.distSqr((int)this.pos.posX, pos.getY(), (int)this.pos.posZ, false)) - size * 0.75F, 0.0F);
		float mult = 1.0F - Math.min(distance / (size * 0.25F), 1.0F);
		return getDownfall() * mult;
	}
