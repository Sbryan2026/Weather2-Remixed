...
public class GuiConfigScrollPanel extends GuiScrollPanel
{
	...
	protected void drawSlotPost(Tessellator tessellatorint, int xPosition, int yPosition, int slot)
	{
		...
		if (hover_string)
		{
			...
			String[] lines = (entry.name + Configuration.NEW_LINE + Configuration.NEW_LINE + ConfigManager.formatCommentForGui(entry.comment, entry.defaultValue, entry.type, entry.showMin, entry.showMax, entry.min, entry.max) + (entry.requiresRestart ? Configuration.NEW_LINE + TextFormatting.RED + "" + TextFormatting.BOLD + "Requires full game restart for changes to take effect" : entry.requiresWorldRestart ? Configuration.NEW_LINE + TextFormatting.RED + "" + TextFormatting.BOLD + "Requires world reload for changes to take effect" : "") + (entry.hasPermission ? "" : Configuration.NEW_LINE + TextFormatting.RED + "" + TextFormatting.BOLD + "Higher permission level required") + Configuration.NEW_LINE + Configuration.NEW_LINE + TextFormatting.GRAY + "" + TextFormatting.ITALIC + "On Text Box" + Configuration.NEW_LINE + TextFormatting.GRAY + "" + TextFormatting.ITALIC + (entry.type == 7 ? "Shift & Left Click: Switch to true/false" : "Shift & Left Click: Reset to original value") + Configuration.NEW_LINE + TextFormatting.GRAY + "" + TextFormatting.ITALIC + "Shift & Right Click: Reset to default value" + Configuration.NEW_LINE  + TextFormatting.BLUE + "" + TextFormatting.ITALIC + entry.registryName).replaceAll(Configuration.NEW_LINE, "\n").split("\\n");
