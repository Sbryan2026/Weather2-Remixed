...
public class SimpleVolumetricMesh
{
    ...
    protected static FloatBuffer createVertices(int quality)
    {
        ...
        for (int lat = 0; lat < lat_bands; lat++)
        {
            ...
            for (int lon = 0; lon < long_bands; lon++)
            {
                ...
                buffer.put(vertex_1).put(vertex_2).put(vertex_3);
                buffer.put(vertex_3).put(vertex_4).put(vertex_1);
            }
        }
    ...
    {
        return new float[] {
            (float) (Maths.fastSin(theta) * Maths.fastCos(phi)),
            (float) Maths.fastCos(theta),
            (float) (Maths.fastSin(theta) * Maths.fastSin(phi))
        };
    }
}